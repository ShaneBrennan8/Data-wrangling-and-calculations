...
# Combine the auto and manual files on the scan_image column. Make sure not to mess this up by using the scan_id column
...
############### Manual only count #############
# Remember to remove dumps and add _m
A_C2 = read.csv("Wanteddata_manaul.csv", head=TRUE, sep=",")
...
write.csv(dta1, file = "Manual Counts for individual parasites.csv")



############### Auto count only ##########
# Remember to remove dumps
A_C2 = read.csv("Wanteddata_auto.csv", head=TRUE, sep=",")
...
write.csv(dta2, file = "Auto Counts for individual parasites.csv")
...
A_C = read.csv("Wanteddata_auto_and_manual_combined.csv", head=TRUE, sep=",")
...
############### FP SUM ###############
#At this point you merge datasets together
...
write.csv(OE_TP_Sum, file = "TP Oxyuris_equi.csv")
...
write.csv(S_TP_Sum, file = "TP Stronglye.csv")
...
write.csv(A_TP_Sum, file = "TP Anoplocephala.csv")
...
write.csv(PE_TP_Sum, file = "TP parascaris_equorum.csv")
...
write.csv(SW_TP_Sum, file = "TP strongyloides_westeri.csv")
...
### code for merging tp remainder datasets
# Not working as of yet. 
library(dplyr)
df_merged <- inner_join(dta1, dta2, by = 'scan_id')
write.csv(df_merged, file = "FP, FN tp remainder innder joined")
